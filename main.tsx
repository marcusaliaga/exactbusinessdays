...
function isMobileViewport(){
  ...
  const width = Math.floor(vv?.width || window.innerWidth || document.documentElement.clientWidth || 0);
  return width <= 760;
}
...
function Info({text}:{text:string}){
  ...
  const openTip = () => {
    ...
    const sideGap = 14;
    const width = Math.min(245, viewportWidth - sideGap * 2);
  ...
  useEffect(() => {
    ...
    const autoClose = window.setTimeout(close, mobileOpen ? 5200 : 0);
  ...
  const desktopTip = open ? createPortal(
    <span
  ...
  const mobileTip = mobileOpen ? createPortal(
    <span className="mobileTipLayer" onClick={closeTip} role="presentation">
      <span className="mobileTip" role="tooltip" onClick={(e) => e.stopPropagation()}>
        <span className="mobileTipTitle"><span className="mobileTipIcon">i</span>Info</span>
        <span className="mobileTipText">{text}</span>
        <span className="mobileTipHint">Tap anywhere to close.</span>
      </span>
    </span>,
    document.body
  ) : null;

  return <span className="infoWrap" onMouseEnter={() => { if (!isMobileViewport()) openTip(); }} onMouseLeave={() => { if (!isMobileViewport()) setOpen(false); }}>
    <span
      ref={iconRef}
      className="infoIcon"
      role="button"
      ...
      onClick={(e) => { e.preventDefault(); e.stopPropagation(); (open || mobileOpen) ? closeTip() : openTip(); }}
      onPointerDown={(e) => e.stopPropagation()}
      onFocus={() => { if (!isMobileViewport()) openTip(); }}
      onBlur={() => { if (!isMobileViewport()) setOpen(false); }}
      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); (open || mobileOpen) ? closeTip() : openTip(); } }}
    >i</span>
    {desktopTip}
    {mobileTip}
  </span>;
